...
struct MomentImageRow: View {
    ...
    func loadImages() {
        for d in images.data.sorted(by: { $0.position < $1.position }).prefix(1) {
    ...
    var body: some View {
        HStack {
            ForEach(
                imageObjects.sorted(by: { $0.position < $1.position }).prefix(1), id: \.position
            ) {
                imageObject in
                imageObject.image
                    .resizable()
